(function () {
    ...
    angular
        .module('smartLpcApp')
        .controller('ContactsUpdateController', ContactsUpdateController);

    ContactsUpdateController.$inject = ['entity', '$state', '$http', '$uibModal', '$scope', '$rootScope', '$stateParams', 'Contacts', 'Lookups', 'Departments', 'User'];

    function ContactsUpdateController(entity, $state, $http, $uibModal, $scope, $rootScope, $stateParams, Contacts, Lookups, Departments, User) {
        ...
        var unsubscribe = $rootScope.$on('smartLpcApp:contactsUpdate', function (event, result) {
            vm.contacts = result;
        });
        ...
        $rootScope.$watch(function () {
            ...
            if ($rootScope.relationships == null) {
                ...
                if (angular.equals(vm.currrentOBJ.elementID, 'field_vm.contacts.contactCompany')) {
                    ...
                    console.log("related Contacts size " + vm.contactDTO.contactRelationships.length);
                } else {
...
//            vm.contactDTO.contactRelationships = vm.relatedContacts;
        ...
        $scope.related = [];
        console.log("vm.relatedContacts : " + JSON.stringify(vm.relatedContact));
        /*for (var i = 0; i < vm.contactDTO.contactRelationships.length; i++) {

         $scope.related.push({'id': 'related' + i});
         console.log("related pushed . ." + $scope.related.length);
         }*/
