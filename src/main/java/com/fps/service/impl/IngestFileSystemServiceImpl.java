    ...
    public Set<IngestFileSystem> newIngests() {
        ...
        for (String project : projectsToIngest) {
            ...
            if (alfrescoTitle2 != null && !alfrescoTitle2.isEmpty()) {
                ...
                final String getProjectSQL = "select id,name,images_location,images_location_remote from projects where alfresco_title_1='" + TITLE_1 + "' and alfresco_title_2='" + TITLE_2 + "'";

                ProjectsIngestDTO projects = null;
                try {
                    projects = jdbcTemplate.queryForObject(getProjectSQL, new Object[]{}, new BeanPropertyRowMapper<>(ProjectsIngestDTO.class));

                    final String totalImageCountCommand = sshCommand.concat(" find ").concat(ingest_new.getTextValue().concat(TITLE_1).concat("/").concat(TITLE_2).concat(Constants.NEW).concat(" -type f -print | wc -l"));
                    ...
                    if (totalImages.equals("0")) {
                        IngestFileSystem ingestFileSystem = new IngestFileSystem(null, TITLE_1.concat("/").concat(TITLE_2), false, Constants.WARNING, "0", "No files present to be ingested", null, source, null, null, null);
                    ...
                    // Display image count related to each folder
                        ...
                        Ingests existsInDB = ingestsRepository.findByAlfrescoTitle1AndAlfrescoTitle2(TITLE_1, TITLE_2);

                        // check if entry already exists in db
                        if (existsInDB != null) {
                            // if exists in db and it had been started , don't display that project
                            if (existsInDB.getStatus().equals(Constants.NOTSTARTED)) {
                                final String params[] = {sshCommand, (ingest_new.getTextValue()).concat(TITLE_1).concat("/").concat(TITLE_2).concat(Constants.NEW)};
                                ...
                                ingestFileSystem = new IngestFileSystem(existsInDB.getId(), project.concat("/").concat(alfrescoTitle2).trim(), true, Constants.NOTSTARTED, totalImages, warning, individualFolderCount, source, null, null, null);
                                allNewIngest.add(ingestFileSystem);
                            } else {
                                continue;
                            ...
                            ingestFileSystem = new IngestFileSystem(currentSaved.getId(), TITLE_1.concat("/").concat(TITLE_2), true, Constants.NOTSTARTED, totalImages, warning, individualFolderCount, source, null, null, null);
                            // add ingest to view
                            allNewIngest.add(ingestFileSystem);
                    ...
                    projects = null;
                    IngestFileSystem ingestFileSystem = new IngestFileSystem(null, TITLE_1.concat("/").concat(TITLE_2), false, Constants.ERROR, "0", "Check folder Structure & Spell Checks", null, source, null, null, null);
                    // add ingest to view
                    allNewIngest.add(ingestFileSystem);
                }
...
// find /mnt/ingest/new/Charlie_Banks_Proj/Charlie_Banks -type f -print | wc -l
    ...
    public void pause(Ingests ingests) {
        ...
        String pauseCommand = "ssh " + username.getTextValue() + "@" + ingests.getIngestProcessor() + " kill -TSTP " + ingests.getId();
    ...
    public void resume(Ingests ingests) {
        ...
        final Lookups username = lookupsRepository.findByConfigAndKey(Constants.CONFIG, Constants.USERNAME);
        String resumeCommand = "ssh " + username.getTextValue() + "@" + ingests.getIngestProcessor() + " kill -CONT " + ingests.getId();
    ...
    // Scan each folder for image count and compare with other folders for same count
    private List<String> eachFolderImageCount(String[] params) {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                final String command2 = params[0].concat(" find ").concat(params[1].concat("/").concat(line)).concat(" -type f -print | wc -l");
    ...
    public Ingests progress(Long id) {
        ...
        final String getCountCommand = "ssh " + username.getTextValue() + "@" + ingests.getIngestProcessor() + " grep --regexp=\"$\" --count " + logFileLocation.getTextValue() + logFileName;
        System.out.println(
            "file command : " + getCountCommand);
        ...
        //ssh root@10.0.1.105 grep --regexp="$" --count /opt/smartlpc_ingests/JobControl/logs/84.log
        // return ingests;
    }
