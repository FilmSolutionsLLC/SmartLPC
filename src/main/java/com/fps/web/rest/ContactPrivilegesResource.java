...
public class ContactPrivilegesResource {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new contactPrivileges, or with status 400 (Bad Request) if the contactPrivileges has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @RequestMapping(value = "/contact-privileges",
        method = RequestMethod.POST,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<ContactPrivileges> createContactPrivileges(@RequestBody ContactPrivileges contactPrivileges) throws URISyntaxException {
        log.debug("REST request to save ContactPrivileges : {}", contactPrivileges);
        if (contactPrivileges.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert("contactPrivileges", "idexists", "A new contactPrivileges cannot already have an ID")).body(null);
        }
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated contactPrivileges,
     * or with status 400 (Bad Request) if the contactPrivileges is not valid,
     * or with status 500 (Internal Server Error) if the contactPrivileges couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @RequestMapping(value = "/contact-privileges",
        ...
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<ContactPrivileges> updateContactPrivileges(@RequestBody ContactPrivileges contactPrivileges) throws URISyntaxException {
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of contactPrivileges in body
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the contactPrivileges, or with status 404 (Not Found)
     */
    @RequestMapping(value = "/contact-privileges/{id}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<ContactPrivileges> getContactPrivileges(@PathVariable Long id) {
        ...
        return Optional.ofNullable(contactPrivileges)
    ...
    public ResponseEntity<Void> deleteContactPrivileges(@PathVariable Long id) {
        ...
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert("contactPrivileges", id.toString())).build();
    }

    /**
     * SEARCH  /_search/contact-privileges?query=:query : search for the contactPrivileges corresponding
     * to the query.
     *
     * @param query the query of the contactPrivileges search
     * @return the result of the search
     */
    @RequestMapping(value = "/_search/contact-privileges",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<ContactPrivileges>> searchContactPrivileges(@RequestParam String query, Pageable pageable)
        throws URISyntaxException {
        ...
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/contact-privileges");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
